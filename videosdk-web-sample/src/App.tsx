    ...
    return () => {
      ...
      if (hasInitialized.current) {
        try {
          ...
          if (info && info.isInMeeting) {
            // Leave the session first so the user does not remain as a ghost participant
            zmClient.leave()
              .catch((err: any) => {
                console.warn('Error leaving Zoom Session on unmount:', err);
              })
              .finally(() => {
